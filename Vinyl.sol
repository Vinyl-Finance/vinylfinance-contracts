...
contract Vinyl is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 72h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 8000 ether;
    // Initial distribution for the day 0-5 TOMB-WFTM LP -> TOMB pool
    uint256 public constant INITIAL_TOMB_POOL_DISTRIBUTION = 28000 ether;
    // Distribution for airdrops wallet
    uint256 public constant INITIAL_AIRDROP_WALLET_DISTRIBUTION = 4000 ether;
    ...
    uint256 public burnThreshold = 1.10e18; // RJS not sure what the value is? But if the price of tomb is below it, taxes are NOT sent to the collector, but rather burned. So should be somewhat low...
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("VINYL Token", "VINYL") {
    ...
    // transfer to receipt minus the tax which goes to the collector
    function _transferWithTax(
        ...
        if(burnTax) {
            ...
            _transfer(sender, taxCollectorAddress, taxAmount); // _transfer is a ERC20Burnable function
        }
    ...
    // RJS setup the initial reward pools
    /**
