...
contract ShareWrapper {
    ...
    // updates global state as well as the individual address' total
...
contract Studio is ShareWrapper, ContractGuard {
    using SafeERC20 for IERC20;
    ...
    // handles updating an address' rewards in its Masonseat struct
    ...
    function initialize(
        ...
        // 1 epch = 6 hours
        // RJS Studio lockup peroids defined here
        withdrawLockupEpochs = 3; // Lock for 3 epochs (18h) before release withdraw
        rewardLockupEpochs = 1; // Lock for 3 epochs (6h) before release claimReward
    ...
    // RJS check that the address, aka "mason", epcok start time + the widthdrawl lock up period is less than the current epock
    ...
    // RJS check that the address, aka "mason", epcok start time + the reward lock up period is less than the current epock
    ...
    // called by address to stake into protocol
    ...
    function withdraw(uint256 amount) public override onlyOneBlock masonExists updateReward(msg.sender) {
        ...
        claimReward();
        super.withdraw(amount); // ShareWrapper's withdrawl function which updates global state as well as individual address'
    ...
    // called by address to claim rewards. Resets the address' lockup timer and the total rewards accumilated before sending to address
    ...
    // called by Treasury to mark end / beging of epoch
    // moves tomb tokens from treasury to the masonary's reward pool?
